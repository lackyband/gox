...
func main() {
	...
	if err != nil {
		fmt.Printf("Failed to create spot listenKey: %v\nResponse body: %s\n", err, string(spotListenKeyResp))
		fmt.Println("Skipping user data stream tests due to listenKey failure")
	} else {
		...
		if err := json.Unmarshal(spotListenKeyResp, &spotListenKeyData); err != nil {
			...
			fmt.Println("Skipping user data stream tests due to parse failure")
		} else {
